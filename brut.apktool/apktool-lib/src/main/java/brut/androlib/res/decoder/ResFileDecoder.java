...
import com.iw.core.apk.ApkInfo;
import com.iw.core.apk.ApkInfo.ApkResourceKey;
...
public class ResFileDecoder {
	...
	public void decode(ResResource res, Directory inDir, ApkInfo info)
		...
		try {
			...
			if (typeName.equals("raw")) {
				decode(inDir, inFileName, baos, outFileName, "raw");
				info.addRawResource(new ApkResourceKey(typeName, outFileName),
						baos.toByteArray());
				return;
			}
			if (typeName.equals("drawable") || typeName.equals("mipmap")) {
				if (inFileName.toLowerCase().endsWith(".9.png")) {
					...
					try {
						...
						decode(inDir, inFileName, baos, outFileName, "9patch");
						info.addRawResource(new ApkResourceKey(typeName,
								outFileName), baos.toByteArray());
						return;
			...
			// All others are xmls?
			baos = new ByteArrayOutputStream();
			decode(inDir, inFileName, baos, outFileName, "xml");
			info.addRawResource(new ApkResourceKey(typeName, outFileName),
					baos.toByteArray());
	...
	public void decode(Directory inDir, String inFileName, OutputStream out,
