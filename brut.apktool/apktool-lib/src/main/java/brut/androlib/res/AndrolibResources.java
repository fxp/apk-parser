...
final public class AndrolibResources {
	...
	public ResPackage loadMainPkg(ResTable resTable, ExtFile apkFile)
		...
		// TODO why there may be 2 packages?
		switch (pkgs.length) {
		case 1:
			pkg = pkgs[0];
			break;
	...
	public void decode(ResTable resTable, ExtFile apkFile, File outDir,
			ApkInfo info) throws AndrolibException {
		...
		try {
			...
			info.setRawAndroidManifest(FileUtil.getStreamString(out.getFileInput("AndroidManifest.xml"), null));
		...
		for (ResPackage pkg : resTable.listMainPackages()) {
			...
			for (ResResource res : pkg.listFiles()) {
				fileDecoder.decode(res, in,info);
			}
	...
	public void aaptPackage(File apkFile, File manifest, File resDir,
		...
		cmd.add("/home/fxp/android-sdk-linux_x86/platform-tools/aapt");
		cmd.add("p");
		if (update) {
			cmd.add("-u");
		}
