...
//					DB_PASSWORD);
		...
		Set apkNames=new HashSet();
		// Open csv file, read file create date
		try {
			while (csvReader.readRecord()) {
				// Get Record detail
				String appNamesTemp = csvReader.get("AppName");
				
				String[] appNames = appNamesTemp.trim().split("/");
				for(String appName:appNames){
					String[] extNames=splitApkName(appName);
					for(String extName:extNames){
						if(extName.equals(""))
							continue;
						apkNames.add(extName);
						importedReport++;
...
*/
		...
		importedReport=0;
		String[] allNames= (String[]) apkNames.toArray(new String[apkNames.size()]);

		int devideInterval=10000;
		BufferedWriter out = new BufferedWriter(new FileWriter(logName+"_"+importedReport+"-"+devideInterval));
		for(String name:allNames){
			out.write(name);
			out.newLine();
			importedReport++;
			if(importedReport%devideInterval==0){
				out.close();
				out = new BufferedWriter(new FileWriter(logName+"_"+importedReport+"-"+(importedReport+devideInterval)));
			}
		}
	}
